#!/usr/bin/env python3
"""
Unified Legal Scraper - Consolidated from both scraper.py and scraper/ modules
Exports data in JSONL format as requested
"""
...
# ==============================
def export_to_jsonl(articles: List[Dict[str, Any]], output_file: str):
    """
    ...
    logger.info(f"Успешно экспортировано {len(articles)} статей в {output_file}")
...
def scrape_code(strategy: BaseStrategy, start_url: str, limit: int, output_file: str):
    ...
    # === EXPORT TO JSONL ===
    export_to_jsonl(articles, output_file)
    print(f"Сохранено → {Path(output_file).resolve()}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", type=str, default=None,
                        help="Путь к выходному файлу (JSONL format)")
    ...
    # Определяем имя файла - теперь в формате JSONL
    default_filename = f"data/raw/{args.code}_rf_articles.jsonl"
