...
if BACKEND == "mlx":
    ...
    logger.info("Apple Silicon detected → MLX-LM")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--log_file", default="fine_tuning.log", help="Path to log file")
    ...
    if not Path(args.data).exists():
        error_msg = f"Training data file not found: {args.data}"
    ...
    if not Path(args.model).exists() and not args.model.startswith("http"):
        ...
        try:
            ...
            error_msg = f"Model not found: {args.model}"
            logger.error(error_msg)
