...
def scrape_all(limit_per_code: int = 10000):
    ...
    for code, (name, base_url) in CONFIG.items():
        ...
        if output_file.exists():
            print(f"  Файл уже есть → пропуск")
            continue

        strategy = ZakonRFStrategy(base_url)
        links = strategy.get_links(limit_per_code)
        ...
        for i, url in enumerate(links, 1):
            ...
            try:
                ...
                if len(article["content"]) > 100:
                    ...
                    seen.add(url)
                print(f"    [{i}/{len(links)}] {article['title'][:70]}...")
