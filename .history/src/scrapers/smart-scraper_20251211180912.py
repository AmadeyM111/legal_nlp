...
def get_balanced_articles(code: str, base_url: str):
    logget.info(f"Собираем [{code.upper()}] — 150 статей с балансом по разделам...")
    result = []
    seen_urls = set()
    article_id = 1
    ...
    print(f"[{code.upper()}] Готово: {lev(result)} статей")
    ...
    if not chapter_links:
        print(f" Оглавление не найдено - используем прямой перебор статей по номерам")
