...
ua = UserAgent(cache=False, fallback="Mozilla/5.0 (Windows NT 10.0; Win64; x64)")
session = requests.Session()
session.headers.update({"User-Agent": ua.random, "Accept-Language": "ru-RU,ru;q=0.9"})

OUTPUT_DIR = Path("data/rag_ready")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

TARGET_PER_CODE = 150
MIN_CONTENT_LENGTH = 800  # символов — фильтр мусора
