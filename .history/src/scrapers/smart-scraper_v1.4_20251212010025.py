...
@retry(
    ...
    wait=wait_exponential(multiplier=2, min=5, max=15),  # Увеличены задержки
...
def fetch(url: str) -> str:
    ...
    # Случайная задержка для избегания антибот-проверки
    delay = random.uniform(5.0, 10.0)
    logger.info(f"Задержка {delay:.2f} секунд для избежания антибот-проверки")
    time.sleep(delay)
...
@retry(
    ...
    wait=wait_exponential(multiplier=1, min=3, max=8),  # Увеличены задержки
...
def get_balanced_articles(code: str, base_url: str) -> list[dict]:
    ...
    if len(result) < TARGET_PER_CODE:
        ...
        while len(result) < TARGET_PER_CODE and attempts_without_success < max_empty_attempts:
            ...
            # Дополнительная случайная задержка между запросами
