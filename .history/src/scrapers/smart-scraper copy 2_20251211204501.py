...
base_url = "https://www.zakonrf.info/nk/"
result = []
...
def get_balanced_articles(code: str, base_url: str) -> list[dict]:
    ...
    chapter_links = list(dict.fromkeys(chapter_links))[:30]  # максимум 30 глав
        ...
        if not chapter_links:
            print(f"    Оглавление не найдено - перебор по номерам")
        ...
        print(f"  Обрабатываем раздел: {chapter_url.split('/')[-2] if '/' in cgapter_url else "главная"}")
