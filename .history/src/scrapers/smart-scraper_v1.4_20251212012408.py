...
def fetch(url: str) -> str:
    """Загрузка страницы без автоматических повторных попыток - только с таймаутом"""
...
def parse_article_safely(url: str) -> dict:
    """Безопасное извлечение статьи с обработкой ошибок"""
    logger.info(f"Парсинг статьи: {url}")
    
    try:
        html = fetch(url)
    except Exception as e:
        logger.warning(f"Не удалось получить HTML для {url}: {e}")
        return None
...
def get_balanced_articles(code: str, base_url: str) -> list[dict]:
    ...
    try:
        ...
        logger.error(f"[{code.upper()}] Не удалось загрузить главную страницу: {e}")
        return []
    ...
    for chapter_url in chapter_links:
        ...
        try:
            ...
            logger.warning(f"Не удалось загрузить раздел {chapter_url}: {e}")
            continue
    ...
    format="%(asctime)s | %(levelname)s | %(message)s",
    datefmt="%H:%M:%S",
    filename="scraper.log"  # Это записало бы логи в файл
)
...
if __name__ == "__main__":
    ...
    for code, base_url in CONFIG.items():
        ...
        out_file = OUTPUT_DIR / f"{code}_100.json"  # Изменено на 100
