...
class PravoScraperV2:
    ...
    def extract_article_number(self, title: str) -> Optional[str]:
        """Извлекает номер статьи из заголовка"""
        import re
        match = re.search(r'Статья\s+(\d+(?:\.\d+)*)', title)
        return match.group(1) if match else None
    
    def clean_article_content(self, article: Dict[str, Any], include_title: bool = True) -> str:
        """Очищает и форматирует содержимое статьи
        
        Args:
            article: Словарь статьи
            include_title: Если True, добавляет заголовок в начало content.
                          Рекомендуется True для векторного поиска, т.к. title 
                          не индексируется отдельно и должен быть в content для лучшей релевантности.
        """
        content_parts = []
        
        # Добавляем заголовок (опционально)
        # ВАЖНО: title в content улучшает векторный поиск, т.к. в rag_system.py
        # индексируется только content, а title хранится только в метаданных
        if include_title:
